...
public class TriggerScalePropertiesDialog extends Dialog {
    
    private final List<SimpleTriggerScale> triggers;
    ...
    private TextView heightText, widthText;
    ...
    public TriggerScalePropertiesDialog(LevelCreatorActivity activity, GridObject gridObject) {
        super(activity);
        this.activity = activity;
        this.gridObjects = new ArrayList<>();
        this.gridObjects.add(gridObject);
        this.triggers = new ArrayList<>();
        this.triggers.add((SimpleTriggerScale) gridObject.getSimpleObject());
        
        setContentView(R.layout.dialog_scale_trigger_properties);
    ...
    public TriggerScalePropertiesDialog(LevelCreatorActivity activity, List<GridObject> gridObjects) {
        super(activity);
        this.activity = activity;
    ...
    private void loadCurrentValues() {
        ...
        heightText.setText(getSharedValueOrDashForMultipliers(triggers, SimpleTriggerScale::getTargetHeight));
        widthText.setText(getSharedValueOrDashForMultipliers(triggers, SimpleTriggerScale::getTargetWidth));
    }

    private String getSharedValueOrDash(List<SimpleTriggerScale> triggers, java.util.function.Function<SimpleTriggerScale, Object> valueExtractor) {
        if (triggers.isEmpty()) return "-";
        
        SimpleTriggerScale firstTrigger = triggers.get(0);
    ...
    private String getSharedValueOrDashForMultipliers(List<SimpleTriggerScale> triggers, java.util.function.Function<SimpleTriggerScale, Double> valueExtractor) {
        if (triggers.isEmpty()) return "-";
        ...
        if (allSameValue) {
            return String.valueOf((int)(firstValue * 10));
        } else {
            return "-";
        }
    }

    private int getCurrentValueOrMin(List<SimpleTriggerScale> triggers, java.util.function.Function<SimpleTriggerScale, Integer> valueExtractor, int minValue) {
    ...
    private double getCurrentValueOrMinDouble(List<SimpleTriggerScale> triggers, java.util.function.Function<SimpleTriggerScale, Double> valueExtractor, double minValue) {
        if (triggers.isEmpty()) return minValue;
    ...
    private void setupButtonListeners() {
        ...
        // Height controls
        setupButtonWithRepeat(heightUpBtn, () -> adjustHeight(MULTIPLIER_INCREMENT), () -> adjustHeight(MULTIPLIER_INCREMENT));
        setupButtonWithRepeat(heightDownBtn, () -> adjustHeight(-MULTIPLIER_INCREMENT), () -> adjustHeight(-MULTIPLIER_INCREMENT));
        
        // Width controls
        setupButtonWithRepeat(widthUpBtn, () -> adjustWidth(MULTIPLIER_INCREMENT), () -> adjustWidth(MULTIPLIER_INCREMENT));
        setupButtonWithRepeat(widthDownBtn, () -> adjustWidth(-MULTIPLIER_INCREMENT), () -> adjustWidth(-MULTIPLIER_INCREMENT));
    ...
    private void adjustDelay(int increment) {
        ...
        for (SimpleTriggerScale trigger : triggers) {
    ...
    private void adjustGroupId(int increment) {
        ...
        for (int i = 0; i < triggers.size(); i++) {
            SimpleTriggerScale trigger = triggers.get(i);
    ...
    private void adjustHeight(double increment) {
        if (triggers.isEmpty()) return;
        double currentHeight = getCurrentValueOrMinDouble(triggers, SimpleTriggerScale::getTargetHeight, MULTIPLIER_MIN);
        double actualIncrement = getAcceleratedIncrement(increment);
        double newHeight = Math.max(MULTIPLIER_MIN, Math.min(MULTIPLIER_MAX, currentHeight + actualIncrement));
        
        for (SimpleTriggerScale trigger : triggers) {
            trigger.setTargetHeight(newHeight);
        }
        heightText.setText(String.valueOf((int)(newHeight * 10)));
        saveStateAndRefresh();
    }
    
    private void adjustWidth(double increment) {
        if (triggers.isEmpty()) return;
        double currentWidth = getCurrentValueOrMinDouble(triggers, SimpleTriggerScale::getTargetWidth, MULTIPLIER_MIN);
        double actualIncrement = getAcceleratedIncrement(increment);
        double newWidth = Math.max(MULTIPLIER_MIN, Math.min(MULTIPLIER_MAX, currentWidth + actualIncrement));
        
        for (SimpleTriggerScale trigger : triggers) {
            trigger.setTargetWidth(newWidth);
        }
