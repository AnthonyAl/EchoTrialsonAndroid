...
public class TriggerGravityPropertiesDialog extends Dialog {
    
    private final List<SimpleTriggerGravity> triggers;
    ...
    private TextView gravityText, directionText;
    ...
    private static final double GRAVITY_MIN = 0.0;
    
    public TriggerGravityPropertiesDialog(LevelCreatorActivity activity, GridObject gridObject) {
        super(activity);
        this.activity = activity;
        this.gridObjects = new ArrayList<>();
        this.gridObjects.add(gridObject);
        this.triggers = new ArrayList<>();
        this.triggers.add((SimpleTriggerGravity) gridObject.getSimpleObject());
        
        setContentView(R.layout.dialog_gravity_trigger_properties);
    ...
    public TriggerGravityPropertiesDialog(LevelCreatorActivity activity, List<GridObject> gridObjects) {
        super(activity);
        this.activity = activity;
    ...
    private void loadCurrentValues() {
        ...
        gravityText.setText(getSharedValueOrDash(triggers, trigger -> (int)(trigger.getGravityStrength() * 10)));
        directionText.setText(getSharedValueOrDash(triggers, trigger -> trigger.getGravityDirection() == SimpleTriggerGravity.GravityDirection.DOWN ? "DOWN" : "UP"));
    }

    private String getSharedValueOrDash(List<SimpleTriggerGravity> triggers, java.util.function.Function<SimpleTriggerGravity, Object> valueExtractor) {
        if (triggers.isEmpty()) return "-";
        
        SimpleTriggerGravity firstTrigger = triggers.get(0);
    ...
    private int getCurrentValueOrMin(List<SimpleTriggerGravity> triggers, java.util.function.Function<SimpleTriggerGravity, Integer> valueExtractor, int minValue) {
    ...
    private double getCurrentValueOrMinDouble(List<SimpleTriggerGravity> triggers, java.util.function.Function<SimpleTriggerGravity, Double> valueExtractor, double minValue) {
        if (triggers.isEmpty()) return minValue;
    ...
    private void setupButtonListeners() {
        ...
        // Direction controls (no acceleration needed for toggle)
        directionUpBtn.setOnTouchListener(LevelCreatorActivity.createButtonTapFeedback());
        directionUpBtn.setOnClickListener(v -> toggleDirection());
        directionDownBtn.setOnClickListener(v -> toggleDirection());
    ...
    private void adjustDelay(int increment) {
        ...
        int newDelay = Math.max(0, Math.min(100000, currentDelay + actualIncrement));
        
        for (SimpleTriggerGravity trigger : triggers) {
    ...
    private void adjustSpeed(int increment) {
        ...
        int actualIncrement = getAcceleratedIncrement(increment);
        int newSpeed = Math.max(1, Math.min(100000, currentSpeed + actualIncrement));
    ...
    private void adjustGroupId(int increment) {
        ...
        for (int i = 0; i < triggers.size(); i++) {
            SimpleTriggerGravity trigger = triggers.get(i);
    ...
    private void adjustGravity(double increment) {
        ...
        // Get current value, or start from 0 if values differ
        double currentGravity = getCurrentValueOrMinDouble(triggers, SimpleTriggerGravity::getGravityStrength, 0);
        double actualIncrement = getAcceleratedIncrement(increment);
        double newGravity = Math.max(GRAVITY_MIN, Math.min(GRAVITY_MAX, currentGravity + actualIncrement));
        
        for (SimpleTriggerGravity trigger : triggers) {
            trigger.setGravityStrength(newGravity);
        }
        
        gravityText.setText(String.valueOf((int)(newGravity * 10))); // Display as x10
        saveStateAndRefresh();
    }
    ...
    private void toggleDirection() {
        ...
        // Get current value, or start from DOWN if values differ
        SimpleTriggerGravity.GravityDirection currentDirection = triggers.get(0).getGravityDirection();
        boolean allSameValue = triggers.stream().allMatch(trigger -> trigger.getGravityDirection() == currentDirection);
        
        SimpleTriggerGravity.GravityDirection newDirection;
        if (allSameValue) {
            newDirection = (currentDirection == SimpleTriggerGravity.GravityDirection.DOWN) ? 
                SimpleTriggerGravity.GravityDirection.UP : SimpleTriggerGravity.GravityDirection.DOWN;
        } else {
            newDirection = SimpleTriggerGravity.GravityDirection.DOWN; // If values differ, set to DOWN
        }
        
        for (SimpleTriggerGravity trigger : triggers) {
            trigger.setGravityDirection(newDirection);
        }
        
        directionText.setText(newDirection == SimpleTriggerGravity.GravityDirection.DOWN ? "DOWN" : "UP");
        saveStateAndRefresh();
    }
